...
<div class="rtin-tab">
	<div class="row feature-tab-layout">
	    ...
	    foreach ( $data['tab_items'] as $tab_item_list ) { ?>
			<li class="nav-item"><a class="<?php echo wp_kses_post( $tab_item_list['themestyle'] ); ?> <?php if ( $i == 1 ) { ?>active<?php } ?>" href="#nav-<?php echo esc_html( $i ); ?>" data-toggle="tab" aria-expanded="false"><?php if( !empty($tab_item_list['icon_class']) ) { ?><?php Icons_Manager::render_icon( $tab_item_list['icon_class'], [ 'aria-hidden' => 'true' ] ); ?><?php } ?>
