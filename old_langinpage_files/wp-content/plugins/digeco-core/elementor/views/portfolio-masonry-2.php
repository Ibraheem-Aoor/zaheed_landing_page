...
$thumb_size = 'digeco-size' . rand( 6, 7 ) ;
...
?>
<div class="portfolio-default portfolio-multi-layout-2 portfolio-masonry-<?php echo esc_attr( $data['layout'] );?> <?php if ( $data['all_button'] == 'hide' ) {?>hide-all<?php } ?> rt-isotope-wrapper">
		...
		<div class="<?php echo esc_attr( $col_class ). ' '; echo esc_html( $terms_of_item ); ?> rt-grid-item">
			<div class="rtin-item">
					...
					<a href="<?php the_permalink(); ?>">
						<?php
							if ( has_post_thumbnail() ){
								the_post_thumbnail( ('digeco-size' . rand( 6, 7 ) ), ['class' => 'img-fluid mb-10 width-100'] );
							} else {
								if ( !empty( DigecoTheme::$options['no_preview_image']['id'] ) ) {
									echo wp_get_attachment_image( DigecoTheme::$options['no_preview_image']['id'], ('digeco-size' . rand( 6, 7 ) ) );
								} else {
				...
				<div class="rtin-content">
					...
					<?php } ?>
					<div class="rtin-read"><a href="<?php the_permalink(); ?>"><?php esc_html_e( 'View Project', 'digeco-core' );?></a></div>
					</div>
				</div>
			</div>
		</div>
	...
	<?php } DigecoTheme_Helper::wp_reset_temp_query( $temp ); 
		...
		<?php if ( $data['more_button'] == 'show' ) { ?>
			...
			<div class="portfolio-button col-12">
				<a class="button-gradient-1" href="<?php echo esc_url( $data['see_button_link'] );?>"><?php echo esc_html( $data['see_button_text'] );?><i class="flaticon-next"></i></a></div>
			<?php } ?>
		<?php } ?>
	</div>             
</div>