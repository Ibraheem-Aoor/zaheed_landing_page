	...
	if( get_post_type() != 'page' ) { 
		...
		$share_buttons = array(
			...
			'facebook-f' => array(
				...
				'text' => esc_html__( 'Facebook', 'digeco-core' ),
			),

			'twitter' => array(
				'url'   => 'https://twitter.com/intent/tweet?text='. $post_title .'&amp;url='. $post_link, 
				'text'  => esc_html__( 'Twitter', 'digeco-core' ),
			),

			'google-plus-g' => array(
				'url'   => 'https://plusone.google.com/_/+1/confirm?hl=en&amp;url='. $post_link .'&amp;name='. $post_title,
				'text'  => esc_html__( 'Google+', 'digeco-core' ),
			),

			'linkedin-in' => array(
				'url'   => 'http://www.linkedin.com/shareArticle?mini=true&amp;url='. $post_link .'&amp;title='. $post_title,
				'text'  => esc_html__( 'LinkedIn', 'digeco-core' ),
			),
			
			'whatsapp' => array(
				'url'   => 'https://api.whatsapp.com/send?text='. $post_title . ' – ' . $post_link ,
				'text'  => esc_html__( 'Whatsapp', 'digeco-core' ),
			),

			'stumbleupon' => array(
				'url'   => 'http://www.stumbleupon.com/submit?url='. $post_link .'&amp;title='. $post_title,
				'text'  => esc_html__( 'StumbleUpon', 'digeco-core' ),
			),

			'tumblr' => array(
				'url'   => 'http://www.tumblr.com/share/link?url='. $post_link .'&amp;name='. $post_title,
				'text'  => esc_html__( 'Tumblr', 'digeco-core' ),
			),

			'pinterest-p' => array(
				'url'   => 'http://pinterest.com/pin/create/button/?url='. $post_link .'&amp;description='. $post_title .'&amp;media='. digeco_post_img_src( 'digeco-size1' ),
				'text'  => esc_html__( 'Pinterest', 'digeco-core' ),
			),

			'reddit-alien' => array(
				'url'   => 'http://reddit.com/submit?url='. $post_link .'&amp;title='. $post_title,
				'text'  => esc_html__( 'Reddit', 'digeco-core' ),
			),
			
			'envelope' => array(
				'url'   => 'mailto:?subject='. $post_title .'&amp;body='. $post_link,
				'text'  => esc_html__( 'Share via Email' , 'digeco-core' ),
				'icon'  => 'far fa-envelope',
			),
			'print' => array(
				'url'   => '#',
				'text'  => esc_html__( 'Print', 'digeco-core' ),
				'icon'  => 'fas fa-print',
			),
		);
		
		if ( DigecoTheme::$options['post_share_facebook'] == 0 ){
		unset($share_buttons['facebook-f']);
		}
		if ( DigecoTheme::$options['post_share_twitter'] == 0 ){
		unset($share_buttons['twitter']);
		}
		if ( DigecoTheme::$options['post_share_google'] == 0 ){
		unset($share_buttons['google-plus-g']);
		}
		if ( DigecoTheme::$options['post_share_linkedin'] == 0 ){
		unset($share_buttons['linkedin-in']);
		}
		if ( DigecoTheme::$options['post_share_whatsapp'] == 0 ){
		unset($share_buttons['whatsapp']);
		}
		if ( DigecoTheme::$options['post_share_stumbleupon'] == 0){
		unset($share_buttons['stumbleupon']);
		}
		if ( DigecoTheme::$options['post_share_tumblr'] == 0 ){
		unset($share_buttons['tumblr']);
		}
		if ( DigecoTheme::$options['post_share_pinterest'] == 0 ){
		unset($share_buttons['pinterest-p']);
		}
		if ( DigecoTheme::$options['post_share_reddit'] == 0 ){
		unset($share_buttons['reddit-alien']);
		}
		if ( DigecoTheme::$options['post_share_email'] == 0 ){
		unset($share_buttons['envelope']);
		}
		if ( DigecoTheme::$options['post_share_print'] == 0 ){
